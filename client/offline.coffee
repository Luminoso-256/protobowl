...
import_external = ->
	unless localStorage.sample_urls3
		expanded = []
		...
		localStorage.sample_urls3 = JSON.stringify(expanded[i] for i in fisher_yates(expanded.length))
	
	# each question packet has 1000
	samples = JSON.parse(localStorage.sample_urls3)
	url = samples.shift()
	localStorage.sample_urls3 = JSON.stringify(samples)
	if url
		load_sample url
